...
**Actual fix, tag-based**: `AssetSelection.tag(key, value)` resolves against the *whole deployment's merged asset graph* at evaluation time, not just whatever assets a single component itself built -- so tagging is enough to let one sensor cover both components' assets without either reading the other's state or keys at all. `GENERATED_ASSET_AUTOMATION_TAG_KEY`/`_VALUE`/`GENERATED_ASSET_AUTOMATION_TAGS` (new, next to `GENERATED_ASSET_AUTOMATION_CONDITION` in `cube_dbt_project/component.py`) are applied to every cube/view `AssetSpec` and to `CubeSupersetSyncComponent.get_dataset_asset_spec`'s dataset `AssetSpec`. `cube_sensor`'s `target` changed from `AssetSelection.assets(*[spec.key for spec in specs])` to `AssetSelection.tag(...)`. `CubeSupersetSyncComponent` no longer builds any sensor of its own at all -- the sibling's one sensor covers it, purely by tag, with zero new coupling between the two components.
...
- New `test_generated_asset_automation_condition_does_not_lose_a_code_version_change_while_blocked` in `test_component_integration.py`: materializes the cube once up front (unlike Phase 38's test), changes its title while the dep stays missing across 3 ticks, materializes the dep, asserts exactly one request fires then and never again. Confirmed via revert-and-confirm-fail: reverted just the `.since_last_handled()` addition, watched it fail (`0 == 1`), restored it.
- New `test_build_defs_from_state_registers_a_sensor_targeting_every_cube_and_view_asset` in `test_component_integration.py`: confirms `CubeDbtProjectComponent`'s own (pre-existing, correct) sensor still resolves to every cube/view key now that it's tag-scoped, not key-scoped.
- New `test_dataset_asset_spec_carries_the_shared_automation_tag` in `test_cube_superset_sync_component.py`: `build_defs_from_sibling_state` registers no sensor of its own, and its dataset `AssetSpec` carries `GENERATED_ASSET_AUTOMATION_TAGS`.
- New `test_a_tag_scoped_sensor_from_the_sibling_also_manages_dataset_assets` in `test_cube_superset_sync_component.py` -- the actual cross-component proof, and the one that matters most here: builds the sibling's *real* `Definitions` (`build_defs_from_state`, not just `prepare_state_aware_lookup`, so its tag-scoped `cube_sensor` genuinely exists) merged with the sync component's dataset assets, and confirms there's still exactly *one* `AutomationConditionSensorDefinition` total, resolving to cube, view, *and* dataset keys together against the merged asset graph. Confirmed via revert-and-confirm-fail (stashing both component files): fails on an `ImportError` for `GENERATED_ASSET_AUTOMATION_TAGS` without the fix, which is itself a fair confirmation -- the fix doesn't exist to import at all in the pre-fix tree.
- `mkdocs build --strict` clean.
- Full suite: **159/159 passed** (155 + 4 new tests; the superseded second-sensor test from the first pass was replaced, not kept).
