...
**Fix**: added a second `AutomationConditionSensorDefinition` to `CubeSupersetSyncComponent.build_defs_from_sibling_state`, targeting `specs` (the dataset asset keys), `default_status=RUNNING`, named from `_sync_pool_name(self.dbt_cube_component)` (the same existing slugify helper `pool=` already uses) to stay unique per sync-component instance.
...
- New `test_build_defs_from_state_registers_a_sensor_targeting_every_cube_and_view_asset` (`CubeDbtProjectComponent`, confirms the pre-existing sensor was already correct) and `test_build_defs_from_sibling_state_registers_a_sensor_targeting_every_dataset_asset` (`CubeSupersetSyncComponent`, the actual regression) -- both assert exactly one `AutomationConditionSensorDefinition`, `default_status=RUNNING`, targeting exactly the expected asset keys. The second confirmed via revert-and-confirm-fail the same way.
- `mkdocs build --strict` clean.
- Full suite: **158/158 passed** (155 + 3 new tests).
