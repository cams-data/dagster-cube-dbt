...
## Phase 53 -- two real automation-condition bugs the user found live in production, on the same day: `CubeSupersetSyncComponent`'s dataset assets had no scoped sensor, and `code_version_changed()` silently loses a pending change while blocked

Both caught by the user inspecting a real Dagster deployment, not a crash: (1) noticed the Superset dataset assets' automation evaluations were running under the platform's default sensor, not a scoped one like the cube/view assets get; (2) two real UI evaluations of the same already-materialized view, 30 seconds apart, showed `code_version_changed` flip from True to False with nothing else changing in between -- while the asset's own page still showed a pending code-version change. Asked for a read on both before anything was changed.

### Bug 1: `CubeSupersetSyncComponent.build_defs_from_sibling_state` never registered a scoped `AutomationConditionSensorDefinition`

`get_dataset_asset_spec` already sets `automation_condition=GENERATED_ASSET_AUTOMATION_CONDITION` (Phase 44) -- the condition itself was never wrong. But `CubeDbtProjectComponent.build_defs_from_state`'s own scoped `cube_sensor` (added purely for that component's cube/view assets) was never mirrored into the sync component, so its dataset assets fell to Dagster's single platform-wide `default_automation_condition_sensor` instead -- functionally correct evaluation, but impossible to start/stop/observe independently of every other automation-condition asset in the deployment. Neither component had ANY test coverage of its sensor registration at all -- confirmed by grepping the whole suite for `sensor` before starting: zero matches. That's exactly why this went unnoticed on the (correctly-wired) `CubeDbtProjectComponent` side too, until this same investigation added coverage for it.

**First pass** (superseded, never shipped): a second, independent `AutomationConditionSensorDefinition` in `CubeSupersetSyncComponent.build_defs_from_sibling_state`, targeting `specs` (the dataset asset keys) directly. Worked, but questioned before merging: why two sensors at all, given both components' assets share the exact same `GENERATED_ASSET_AUTOMATION_CONDITION`? A merged single sensor was the actual ask -- but `CubeSupersetSyncComponent` deliberately never builds the sibling's live `Definitions` (`build_defs_from_sibling_state` calls `sibling.prepare_state_aware_lookup`, never `sibling.build_defs_from_state` -- see that method's own docstring), so there's no sibling sensor object to append keys to or merge with, short of reintroducing the cross-component coupling this architecture (`SUPERSET_SYNC_PLAN.md`'s "Architecture decision") exists to avoid.

**Second pass** (superseded, never shipped): tag-based instead. `AssetSelection.tag(key, value)` resolves against the *whole deployment's merged asset graph* at evaluation time, not just whatever assets a single component itself built -- so tagging is enough to let one sensor cover both components' assets without either reading the other's state or keys at all. `GENERATED_ASSET_AUTOMATION_TAG_KEY`/`_VALUE`/`GENERATED_ASSET_AUTOMATION_TAGS` were applied to every cube/view `AssetSpec` and to `CubeSupersetSyncComponent.get_dataset_asset_spec`'s dataset `AssetSpec`; `cube_sensor`'s `target` changed to `AssetSelection.tag(...)`; `CubeSupersetSyncComponent` stopped building any sensor of its own. Worked, and verified cross-component (see the now-removed test that proved it) -- but questioned again before merging, this time on the tag itself: a bundled tag is a silent footgun the moment a subclass overrides `get_cube_asset_spec`/`get_view_asset_spec`/`get_dataset_asset_spec` (a fully legitimate, already-documented override point) without delegating through `super()` -- the fresh `AssetSpec` it builds has no idea the tag exists, so it silently drops out of whatever sensor was targeting it. Automation still runs correctly either way (via the default sensor), so nothing actually breaks -- but "why isn't my custom sensor picking this up" is a confusing thing to have to debug for a property (independent start/stop of these assets) that isn't required for correctness in the first place. The exact same class of override-fragility this codebase has been bitten by twice before (Phase 37/40, for dependency-key derivation) -- just against tags instead of keys.

**Final decision, shipped: no bundled sensor, no bundled tag, at all.** The condition itself (`GENERATED_ASSET_AUTOMATION_CONDITION`) stays -- that's real, useful default behavior. But this library no longer registers any `AutomationConditionSensorDefinition`, and doesn't tag generated specs for one either; these assets are simply picked up by Dagster's own default automation condition sensor, same as any other asset with an `automation_condition` and no more specific sensor claiming it. `CubeDbtProjectComponent.build_defs_from_state` no longer builds `cube_sensor` at all. README's "Advanced: bringing your own automation sensor" section (new) shows the by-hand pattern instead: override the relevant `get_*_asset_spec` methods to add your own tag via `merge_attributes` (not `replace_attributes`, which would drop any tags you already set for other reasons), then target that tag with your own `AutomationConditionSensorDefinition` -- for anyone who actually wants independent start/stop/observability badly enough to accept that maintenance burden themselves, rather than this library taking it on unconditionally for every user.

### Bug 2: `code_version_changed()` is a one-tick pulse, not a persisted state -- the block comment above `GENERATED_ASSET_AUTOMATION_CONDITION` had this backwards since Phase 38

Confirmed directly from dagster's own source, not assumed: `CodeVersionChangedCondition.evaluate` (`declarative_automation/operands/operands.py`) compares `context.cursor` (the code version recorded on the *previous evaluation tick*) against the *current* code version, and unconditionally persists the current version as the new cursor on every single call -- regardless of whether the condition evaluated true, regardless of whether a request went out. That means it can only ever read true on the *one* tick immediately following a version change; by the very next tick, cursor and current already match again.

Phase 38's own regression test (blocking `code_version_changed()` behind `_DEPS_READY` for several ticks, then unblocking) still passes despite this, but doesn't actually prove what its comment claims: that test's cube asset was *never materialized* at all, so its `missing()` branch -- independently true the whole time, and correctly wired since Phase 38 -- fires on its own the moment the dep becomes ready, fully masking whether `code_version_changed()` itself survived. The user's real asset had already been materialized once before its definition changed again, meaning `missing()` was permanently false for it -- the one case that actually exercises `code_version_changed()`'s persistence in isolation, and the one case nothing in this codebase had ever tested.

Reproduced directly against a real `evaluate_automation_conditions` tick sequence before touching anything (matching this project's own established practice, see Phase 7/8/38's comments): materialized the cube once, changed its title while the dbt model dep stayed missing across 4 ticks, then materialized the dep. With the bare `code_version_changed() & _DEPS_READY` in place, the final tick requested **0** runs -- the pending change was silently lost. Not a corner case: any definition edit made while a dependency happens to be mid-run or briefly unavailable for more than one automation tick would permanently drop that change until something else (a further edit, or a manual materialization) came along.

**Fix**: `code_version_changed().since_last_handled() & _DEPS_READY`. Unlike `missing()`, `code_version_changed()` doesn't need a `newly_true()` wrap first -- it's already pulse-shaped on its own -- so `.since_last_handled()` alone is enough to hold that pulse true from the tick it fires until the tick a request is actually handled, surviving any number of blocked ticks in between; the `_DEPS_READY` gate is safe to apply as a plain trailing `&` *outside* the wrap (unlike `missing()`, which needs it *inside* the `newly_true()`) because the persisted state no longer depends on which tick the gate happens to open. Rewrote the incorrect block comment in full rather than patching around it -- it previously stated the exact opposite of what dagster's source and this reproduction both show.

### Verification

- New `test_generated_asset_automation_condition_does_not_lose_a_code_version_change_while_blocked` in `test_component_integration.py`: materializes the cube once up front (unlike Phase 38's test), changes its title while the dep stays missing across 3 ticks, materializes the dep, asserts exactly one request fires then and never again. Confirmed via revert-and-confirm-fail: reverted just the `.since_last_handled()` addition, watched it fail (`0 == 1`), restored it. This is the one piece of this phase that's still in the shipped code -- the bundled-sensor tests from both superseded passes (key-scoped, then tag-scoped) were written, confirmed to actually catch their respective regressions, and then deleted along with the code they were testing once the "no bundled sensor" decision was made -- there's nothing left to assert once nothing is bundled.
- `mkdocs build --strict` clean, including the new README "Advanced: bringing your own automation sensor" section.
- Full suite: **156/156 passed** (155 + 1 net new test -- the `code_version_changed()` persistence fix's regression test is the only addition that survived all three passes of this phase).
